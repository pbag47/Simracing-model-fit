...
impl AcUdpReader {
    ...
    pub async fn connect(
        ...
        debug!(
            "AcRtCarInfo size = {} bytes",
            std::mem::size_of::<AcRtCarInfo>()
        );
        debug!(
            "AcRtLap size = {} bytes",
            std::mem::size_of::<AcRtLap>()
        );
        debug!(
            "AcHandshakePacket size = {} bytes",
            std::mem::size_of::<AcHandshakePacket>()
        );
        debug!(
            "AcHandshakeResponse size = {} bytes",
            std::mem::size_of::<AcHandshakeResponse>()
        );
...
// Vérification au compile-time — si ça ne compile pas,
// recompte les champs et ajuste les _pad
const _: () = assert!(
    std::mem::size_of::<AcRtCarInfo>() == 328,
    "AcRtCarInfo: taille incorrecte, vérifier le padding",
);
